...
@pytest.mark.parametrize(
    ...
    [
        ...
        (decimal.Decimal(1), decimal.Decimal("0.50"), KALSHI_TAKER_FEE_RATE, decimal.Decimal("0.02")),
        # 10 contracts at 50c: 0.07 * 10 * 0.50 * 0.50 = 0.175 → ceil → 0.18
        (decimal.Decimal(10), decimal.Decimal("0.50"), KALSHI_TAKER_FEE_RATE, decimal.Decimal("0.18")),
        # 100 contracts at 30c: 0.07 * 100 * 0.30 * 0.70 = 1.47 → ceil → 1.47
        (decimal.Decimal(100), decimal.Decimal("0.30"), KALSHI_TAKER_FEE_RATE, decimal.Decimal("1.47")),
        # 1 contract at 95c: 0.07 * 1 * 0.95 * 0.05 = 0.003325 → ceil → 0.01
        (decimal.Decimal(1), decimal.Decimal("0.95"), KALSHI_TAKER_FEE_RATE, decimal.Decimal("0.01")),
        # 50 contracts at 65c: 0.07 * 50 * 0.65 * 0.35 = 0.79625 → ceil → 0.80
        (decimal.Decimal(50), decimal.Decimal("0.65"), KALSHI_TAKER_FEE_RATE, decimal.Decimal("0.80")),
        # Zero fee rate
...
def test_calculate_kalshi_commission(
    ...
    """Test Kalshi fee formula: fee = ceil_to_cent(rate × C × P × (1-P))."""
